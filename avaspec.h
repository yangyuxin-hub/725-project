...
typedef void* LVUserEventRef; // 为避免报错，把对extcode.h的引用改为本行
...
namespace AvaSpec { // 括进命名空间以避免SensorType的重名冲突
...
typedef struct
{
    ...
    uint32_t              m_IntegrationDelay;    // 4
    uint32_t              m_NrAverages;		   // 4
...
#define  SETTINGS_RESERVED_LEN ((62 * 1024) - sizeof(uint32_t) - /* CRC calculated by firmware */ \
                                (sizeof(uint16) + /* m_Len */ \
                                sizeof(uint16) +  /* m_ConfigVersion */ \
                                USER_ID_LEN + \
                                sizeof(AvaSpec::DetectorType) + \
...
typedef struct
{
    ...
    AvaSpec::DetectorType            m_Detector;
...
typedef struct
{
    uint32_t	m_TotalScans;  // Internal Storage Size; the size of the scan data buffer
    uint32_t	m_UsedScans;   // Internal Storage Scan Count; the number of used elements
    uint32_t	m_Flags;       // DSTR measurement mode flags as described below;
