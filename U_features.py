...
# 提取电压时域特征
def extract_voltage_time_features(signal):
...
# 处理每个文件夹中的所有电压信号
def process_voltage_folder(folder):
    voltage_signals = load_signals_from_folder(folder)  # 加载电压信号
    features = [extract_voltage_time_features(signal) for signal in voltage_signals]
