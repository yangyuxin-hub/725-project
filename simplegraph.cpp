...
void SimpleGraph::paintEvent(QPaintEvent *event) //画网格
{
	QPainter p(this);
...
void SimpleGraph::CalcDrawParams(double XIncBySec) //按x轴尺度缩放
{
    if (XIncBySec < 1e-15)
    {
        return;
...
void SimpleGraph::SaveData(double *data, int plotCount, int dataCountPerPlot)  //数据 通道数 各个通道的数据量
{
...
void SimpleGraph::MapDataPoints()  // 图表拉伸过后-点重画
{
